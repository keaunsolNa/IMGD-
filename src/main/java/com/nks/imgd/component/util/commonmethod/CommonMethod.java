package com.nks.imgd.component.util.commonmethod;

import static com.nks.imgd.dto.enums.ResponseMsg.*;
import static com.nks.imgd.dto.enums.ResponseMsg.GROUP_CREATE_FAILED;
import static com.nks.imgd.dto.enums.ResponseMsg.GROUP_LIMIT_EXCEEDED;
...
public class CommonMethod {
	...
	public <T> ResponseEntity<ApiResponse<T>> responseTransaction(ServiceResult<T> result) {
		...
		} else if (status.equals(GROUP_LIMIT_EXCEEDED)) {
			return ResponseEntity.status(400).body(ApiResponse.error(GROUP_LIMIT_EXCEEDED));
		} else if (status.equals(GROUP_CREATE_FAILED)) {
			return ResponseEntity.status(400).body(ApiResponse.error(GROUP_CREATE_FAILED));
		} else {
