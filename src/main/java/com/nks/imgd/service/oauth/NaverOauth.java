...
public class NaverOauth implements SocialOauth {
	...
	public String getOauthRedirectUrl() {
		...
		params.put("redirect_uri", naverCallbackUrl);
		SecureRandom random = new SecureRandom();
		String state = new BigInteger(130, random).toString();
		params.put("state", state);
		params.put("grant_type", naverGrantType);

		String parameterString = params.entrySet().stream()
			.map(x -> x.getKey() + "=" + x.getValue())
			.collect(Collectors.joining("&"));

		return naverBaseUrl + "?" + parameterString;
	}
	...
	public String[] requestUserInfo(String accessToken) {
		...
		String userInfo = restTemplate.exchange(naverUserInfoUri, HttpMethod.GET, httpEntity, String.class)
			.getBody();
		...
		if (null == user) {
			...
			user.setLoginType(SocialLoginType.NAVER);
